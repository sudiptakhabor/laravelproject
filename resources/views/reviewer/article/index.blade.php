                    ...
                    <table id="basic-datatable" class="table table-striped table-hover table-dark nowrap" style="width:100%">
                        ...
                        <tbody>
                          @foreach( $rows as $key => $row )
							<?php 
								/* ----- by default setting 10  */
							
								$days 	=	10;
								if(isset($row->review_date[0]->review_last_date))
								{
									$current_date = new DateTime(date('Y-m-d'));
									$datetime2 = new DateTime($row->review_date[0]->review_last_date);
                            ...
                            <tr>
                                ...
                                <td>{{ $row->category->title }}</td>
								 <td>
								{{ (isset($row->review_date[0]->review_last_date)?date('M d,Y', strtotime($row->review_date[0]->review_last_date)):'NA') }}
                                </td>
                                <td>
                                    @if( $row->status == 0 )
